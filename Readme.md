...
**Tier 3** (Advanced): `joinmarket-node`, `storj-node`, `forgejo-node` (self-hosted Git forge + CI runners), `webmin-node` (web system administration), `usermin-node` (per-user web interface) — implemented; `hns-node` (Handshake) — planned
